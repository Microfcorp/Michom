...
if(!empty($_GET['period'])){
    ...
    if(!empty($_GET['start'])){
        $results = mysqli_query($link, "SELECT * FROM michom WHERE ip='".$ipd."' AND `id` >= ".$_GET['start']." AND `id` <= (".$_GET['start']." + ".$period.") ORDER BY id ASC LIMIT " . $period);
    }
	else{
        $results = mysqli_query($link, "SELECT * FROM michom WHERE ip='".$ipd."' ORDER BY id DESC LIMIT " . $period);
        $arrrevers = true;
	}
}
else{
    $results = mysqli_query($link, "SELECT * FROM michom WHERE ip='".$ipd."'");
}
...
while($row = $results->fetch_assoc()) {
	...
	elseif($_GET['type'] == "tempul" or $_GET['type'] == "temperbatarey" or $_GET['type'] == "temp"){
		$par = "C";
		if($row['temp'] != ""){
            $data[] = $row['temp'];
            if(!empty($_GET['mode']))
